...
void  QPSK::R_Adjust(double *tmp){
    *tmp -= rMove;
}
void  QPSK::L_Adjust(double *tmp){
    *tmp += 0.4;
}
void  QPSK::D_Adjust(double *tmp){
    *tmp -= 0.3;
}
void QPSK::wallplot(){
...
int QPSK::hex2int(char a,char b,char c,char d)
{
    /*
      *   eg. ff to 1111 1111
      */
...
double QPSK::char2double(char *str){
    double  ans = 0;
...
void  QPSK::Draw_line(){
    ...
    for(int i = 0 ; i < num_p/5.0 ; i++){
        glVertex3f(1+*(pdata2+i)/5.0, point2[crr][1]+0.1  , point2[crr][2]);
